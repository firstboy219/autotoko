...
export function PackingMaterialsCard() {
  ...
  async function add() {
    const q = Number(qty);
    if (!materialId || !Number.isFinite(q) || q <= 0) return;
    setBusy(true);
    try {
      await api.post("/costing/packing-materials", { materialId, defaultQuantity: q });
      setMaterialId("");
      setQty("1");
      list.reload();
    } catch (e) {
  ...
  return (
    <Card padded={false} className="mb-5">
      ...
      <div className="px-5 py-3 border-b border-line flex flex-wrap items-end gap-2">
        ...
        <Button variant="filled" onClick={add} disabled={busy || !materialId}>
      ...
      {catalog.data && available.length === 0 && rows.length === 0 && (
        <div className="p-4">
          <InlineAlert tone="info">
            Belum ada bahan baku di katalog. Tambahkan dulu lewat menu Pembelian Stok atau BOM.
          </InlineAlert>
        </div>
      )}
